  ...
  if (errorMessage || !product) {
    return (
      <div className="max-w-xl mx-auto p-6 bg-slate-900 border border-slate-800 rounded-2xl text-center space-y-4">
        ...
        <h3 className="text-base font-bold text-slate-100">{errorMessage || 'Product not found'}</h3>
        <button
    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-900/90 backdrop-blur-md border border-slate-800 rounded-2xl p-6 shadow-2xl flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-6 border-t md:border-t-0 md:border-l border-slate-800 pt-4 md:pt-0 md:pl-6 text-xs text-slate-400">
          <div>
            ...
            <span className="font-mono text-emerald-400 text-xl font-extrabold">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* Card 2: Upcoming Inventory Notice */}
          <div className="bg-slate-900/40 border border-dashed border-slate-800 rounded-2xl p-6 space-y-3">
            <div className="flex items-center gap-2 text-cyan-400 font-bold text-xs">
              <Sparkles className="w-4 h-4" />
              <span>Stock Movement Ledger</span>
            </div>
            <p className="text-xs text-slate-400 leading-relaxed">
              Stock In, Stock Out, and Delivery Challan reductions are decoupled from product specifications and will be recorded in the <strong className="text-slate-200">Inventory Module (Checkpoint 6)</strong>.
            </p>
          </div>
        </div>
              ...
              <div className="flex items-center justify-between p-3 rounded-xl bg-slate-950/50 border border-slate-800/60">
                ...
                {product.currentStock <= product.minimumStock ? (
                  <span className="text-amber-400 font-bold flex items-center gap-1 text-[11px]">
                    <TrendingDown className="w-3.5 h-3.5" /> Reorder Triggered
                  </span>
                ) : (
